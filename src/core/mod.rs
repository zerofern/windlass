...
use crate::types::{AlertPriority, Backoff, Interval, RetryCount, WakeupId};
use actions::Action;
use events::Event;
    ...
    match event {
        ...
        Event::DockerGluetunDied => {
            match &state.vpn {
                ...
                VpnState::Connected { .. } | VpnState::AwaitingTunnel => {
                    warn!(vpn = %state.vpn, qbit = %state.qbit, "Gluetun died unexpectedly — beginning recovery");
                    state.vpn = VpnState::DumpingLogs;
                    actions.push(Action::FetchAndDumpAllLogs);
        ...
        Event::QbitConnectionRefused => {
            if matches!(state.qbit, QbitState::Authenticating { .. }) {
                ...
                debug!(qbit = %state.qbit, "stale QbitConnectionRefused — ignoring");
            }
        }
        ...
        Event::QbitPortSyncSuccess => {
            if let QbitState::SyncingPort { target, .. } = &state.qbit {
                let port = *target;
                if let VpnState::Connected { ip, .. } = &state.vpn {
                    let ip = *ip;
            ...
            if let QbitState::SyncingPort { attempt, cookie, target } = &state.qbit {
                ...
                if attempt < QBIT_SYNC_RETRY_LIMIT {
                    ...
                    actions.push(Action::SendGotifyAlert(
                        ...
                        format!(
                            "⚠️ qBittorrent rejecting port updates (HTTP {err_code}) after {} attempts. Forcing re-auth.",
        ...
        Event::MamConnectabilityObserved(false) => {
            warn!(mam = %state.mam, qbit = %state.qbit, "MAM reports NOT connectable");
            // If ASN is blocked, a human must intervene. Don't attempt recovery.
            if let MamState::AsnBlocked { .. } = &state.mam {
            ...
            match &state.qbit {
                ...
                _ => {
                    let recoveries = state.hard_recoveries.increment();
                    state.hard_recoveries = recoveries;
        ...
        Event::NewTorrentsObserved(current) => {
            let new_names: Vec<String> = current
                .iter()
                .filter(|name| !state.known_torrents.contains(*name))
                .cloned()
                .collect();
        ...
        Event::Wakeup(id) => match id {
            ...
            WakeupId::QbitAuthRetry  => {
                if matches!(state.qbit, QbitState::Authenticating { .. }) {
                    ...
                    debug!(qbit = %state.qbit, "QbitAuthRetry wakeup: no longer authenticating — ignoring");
                }
            }
            WakeupId::QbitSyncRetry => {
                if let QbitState::SyncingPort { cookie, target, .. } = &state.qbit {
