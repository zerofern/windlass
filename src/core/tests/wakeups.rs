...
fn wakeup_heartbeat_checks_mam_connectability() {
    let (_, actions) = process_event(SystemState::initial(), Event::Wakeup(WakeupId::Heartbeat));
    assert!(
        actions
            .iter()
...
fn wakeup_disk_check_checks_disk_space() {
    let (_, actions) = process_event(SystemState::initial(), Event::Wakeup(WakeupId::DiskCheck));
...
fn wakeup_torrent_check_checks_new_torrents() {
    let (_, actions) = process_event(
        SystemState::initial(),
        Event::Wakeup(WakeupId::TorrentCheck),
    );
    assert!(
        actions
            .iter()
...
fn wakeup_retry_port_read_reads_port_files() {
    let (_, actions) = process_event(
        SystemState::initial(),
        Event::Wakeup(WakeupId::RetryPortRead),
    );
