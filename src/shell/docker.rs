...
impl DockerClient {
    ...
    pub fn spawn_file_watcher(&self, tx: mpsc::Sender<Event>) {
        let watch_dir = Path::new(&self.vpn_ip_file)
            .parent()
            .map_or_else(|| "/tmp/gluetun".to_string(), |p| p.to_string_lossy().into_owned());
        spawn_file_watcher_inner(&watch_dir, self.vpn_ip_file.clone(), self.vpn_port_file.clone(), tx);
    }
    ...
    pub async fn restart_gluetun(&self) {
        ...
        if let Err(e) = self.inner.restart_container(&self.gluetun_anchor, Some(options)).await {
...
mod tests {
    ...
    fn read_port_files_missing_ip_file_returns_err() {
        let port_f = write_temp("51820");
        let err = read_port_files("/nonexistent/ip_xyz", port_f.path().to_str().unwrap())
            .unwrap_err();
    ...
    fn read_port_files_missing_port_file_returns_err() {
        let ip_f = write_temp("10.8.0.1");
        let err = read_port_files(ip_f.path().to_str().unwrap(), "/nonexistent/port_xyz")
            .unwrap_err();
    ...
    async fn file_watcher_fires_on_subsequent_writes() {
        ...
        tokio::time::timeout(Duration::from_secs(3), rx.recv())
            .await.expect("timed out on first write").unwrap();
        // Wait for the debounce window to fully settle before the second write.
        tokio::time::sleep(Duration::from_millis(400)).await;
        std::fs::write(&port_path, "51822").unwrap();
        tokio::time::timeout(Duration::from_secs(3), rx.recv())
            .await.expect("timed out on second write — watcher stopped after first event").unwrap();
    }
    ...
    async fn docker_is_container_healthy_returns_false_for_nonexistent() {
        ...
        assert!(!client.is_container_healthy("windlass_test_definitely_absent_xyz").await);
    }
    ...
    async fn docker_discover_dependents_finds_containers_in_network_mode() {
        ...
        docker.create_container(
            ...
            Config::<String> { image: Some("alpine".into()), cmd: Some(vec!["sleep".into(), "30".into()]), ..Default::default() },
        ).await.expect("create anchor");
        docker.start_container(anchor, None::<StartContainerOptions<String>>).await.expect("start anchor");
        ...
        docker.create_container(
            ...
            Config::<String> {
                ...
                host_config: Some(HostConfig { network_mode: Some(format!("container:{anchor}")), ..Default::default() }),
                ..Default::default()
            },
        ).await.expect("create dependent");
        docker.start_container(dependent, None::<StartContainerOptions<String>>).await.expect("start dependent");
        ...
        assert!(found.contains(&dependent.to_string()), "Expected {dependent:?} in {found:?}");
    }
    ...
    async fn docker_fetch_and_dump_logs_creates_log_file() {
        ...
        docker.create_container(
            ...
            Config::<String> {
                image: Some("alpine".into()),
                cmd: Some(vec!["sh".into(), "-c".into(), "echo 'windlass_log_marker'; sleep 30".into()]),
                ..Default::default()
            },
        ).await.expect("create container");
        docker.start_container(container_name, None::<StartContainerOptions<String>>).await.expect("start container");
        tokio::time::sleep(Duration::from_millis(500)).await;

        let dump_client = DockerClient::connect(
            dump_dir.path().to_str().unwrap().to_string(), "/tmp/ip".into(), "/tmp/port".into(),
        ).expect("Docker socket unavailable");
        dump_client.fetch_and_dump_logs(&[container_name.to_string()]).await;

        let _ = docker.stop_container(container_name, Some(StopContainerOptions { t: 0 })).await;
        let _ = docker.remove_container(container_name, Some(RemoveContainerOptions { force: true, ..Default::default() })).await;

        let files: Vec<_> = std::fs::read_dir(dump_dir.path()).unwrap()
            .filter_map(|e| e.ok())
        ...
        assert!(content.contains("windlass_log_marker"), "Missing marker in: {content:?}");
    }
}
