...
pub async fn upsert(pool: &DbPool, mam_id: MamTorrentId) -> Result<i64, DbError> {
    let mam = i64::try_from(mam_id.into_inner()).unwrap_or(i64::MAX);
...
mod tests {
    ...
    async fn upsert_returns_id_and_is_idempotent() {
        ...
        let mam_id = MamTorrentId::try_new(42).unwrap();
    ...
    async fn get_by_mam_id_returns_none_when_missing() {
        let pool = test_pool().await;
        let row = get_by_mam_id(&pool, MamTorrentId::try_new(999).unwrap())
            .await
            .unwrap();
        assert!(row.is_none());
    }
    ...
    async fn get_by_mam_id_returns_row_after_upsert() {
        let pool = test_pool().await;
        let mam_id = MamTorrentId::try_new(7).unwrap();
    ...
    async fn get_all_returns_all_books() {
        let pool = test_pool().await;
        upsert(&pool, MamTorrentId::try_new(1).unwrap())
            .await
            .unwrap();
        upsert(&pool, MamTorrentId::try_new(2).unwrap())
            .await
            .unwrap();
