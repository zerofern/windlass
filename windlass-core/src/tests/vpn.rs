...
fn unexpected_vpn_death_dumps_logs() {
    ...
    let mut state = state;
    let outcome = state.process_event(Event::DockerGluetunDied { at: now() }, now());
...
fn logs_dumped_stops_containers_and_restarts_gluetun() {
    ...
    let outcome = state.process_event(Event::LogsDumped { at: now() }, now());
    let actions = outcome.actions;
    assert!(outcome.state_changed);
...
fn gluetun_healthy_starts_containers() {
    let mut state = SystemState::initial();
    state.vpn = VpnState::Starting;
    let outcome = state.process_event(Event::DockerGluetunHealthy { at: now() }, now());
    let actions = outcome.actions;
    assert!(outcome.state_changed);
